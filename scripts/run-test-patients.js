...
const vsacApiKey = process.env.VSAC_API_KEY
...
for (const file of patientFiles) {
  ...
  try {
    execSync(`fqm-execution reports -m "${measureBundlePath}" -p "${file}" --vs-api-key "${vsacApiKey}" --cache-valuesets -o "${outputFile}"`, { stdio: 'inherit' });
